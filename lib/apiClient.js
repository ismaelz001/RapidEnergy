...
// ========================================
// FASE 1: SNAPSHOTS, ALERTAS Y TAREAS
// ========================================

/**
 * Obtiene tareas de la semana especificada
 * @param {number} semanaOffset - Offset de semanas (0 = esta semana, 1 = próxima, etc.)
 */
export async function getTareasSemana(semanaOffset = 0) {
  const res = await fetch(`${API_URL}/api/fase1/tareas/semana?semana_offset=${semanaOffset}`, {
    headers: getAuthHeaders(),
    cache: "no-store",
  });

  if (!res.ok) {
    console.error("Error obteniendo tareas de la semana");
    return { tareas: [] };
  }
...
/**
 * Actualiza una tarea (marca como completada, cambia estado, etc.)
 */
export async function updateTarea(id, payload) {
  const res = await fetch(`${API_URL}/api/fase1/tareas/${id}`, {
    method: "PUT",
    headers: getAuthHeaders(),
    body: JSON.stringify(payload),
  });
  ...
  if (!res.ok) {
    const errorData = await res.json().catch(() => ({}));
    const message = errorData.detail || "Error actualizando tarea";
    throw new Error(message);
  }
...
/**
 * Obtiene alertas de renovación
 * @param {Object} filters - Filtros (estado, proximos_dias, etc.)
 */
export async function getAlertasRenovacion(filters = {}) {
  const params = new URLSearchParams();
  if (filters.estado) params.append('estado', filters.estado);
  if (filters.proximos_dias) params.append('proximos_dias', filters.proximos_dias);
  if (filters.comercial_id) params.append('comercial_id', filters.comercial_id);

  const res = await fetch(`${API_URL}/api/fase1/alertas/renovacion?${params.toString()}`, {
    headers: getAuthHeaders(),
    cache: "no-store",
  });

  if (!res.ok) {
    console.error("Error obteniendo alertas de renovación");
    return { alertas: [] };
  }
...
/**
 * Actualiza una alerta de renovación
 */
export async function updateAlerta(id, payload) {
  const res = await fetch(`${API_URL}/api/fase1/alertas/renovacion/${id}`, {
    method: "PUT",
    headers: getAuthHeaders(),
    body: JSON.stringify(payload),
  });
  ...
  if (!res.ok) {
    const errorData = await res.json().catch(() => ({}));
    const message = errorData.detail || "Error actualizando alerta";
    throw new Error(message);
  }
...
export async function getDashboardStats() {
  const res = await fetch(`${API_URL}/api/fase1/dashboard/stats`, {
    headers: getAuthHeaders(),
    cache: "no-store",
  });

  if (!res.ok) {
    console.error("Error obteniendo stats del dashboard");
    return null;
  }
...
/**
 * Ejecuta snapshot mensual manualmente (solo CEO/dev)
 */
export async function ejecutarSnapshot() {
  const res = await fetch(`${API_URL}/api/fase1/snapshot/ejecutar`, {
    method: "POST",
    headers: getAuthHeaders(),
  });
  ...
  if (!res.ok) {
    const errorData = await res.json().catch(() => ({}));
    const message = errorData.detail || "Error ejecutando snapshot";
    throw new Error(message);
  }
